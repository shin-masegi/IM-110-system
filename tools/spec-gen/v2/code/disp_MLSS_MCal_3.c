uint8_t disp_MLSS_MCal_3(uint8_t flash_flg, uint8_t msg_flag, uint8_t batvol)
{
	if (lcd_init(0) != LCD_OK)
	{					// LCD表示クリア(＆LCD表示バッファ更新OKか確認)
		return DISP_NG; // 更新NGのため終了
	}
	LS027_disp_icon(0, 1, (uint8_t *)t_3p_cal);	//3点校正
	...
	if (msg_flag) {
		...
		default:
			...
			LS027_disp_number(41, 18, 1, MLSS_Cal_SetVal_2, 0); //中間校正設定値
		...
		case MEAS_MODE_SS:
			...
			LS027_disp_icon(44, 11, (uint8_t *)l_mgl); // "mg/L"
			LS027_disp_number(41, 18, 1, SS_Cal_SetVal_2, 0); //中間校正設定値
		...
		case MEAS_MODE_TR:
			...
			LS027_disp_number(41, 18, 1, TR_Cal_SetVal_2, 1); //中間校正設定値 (小数点1桁)
