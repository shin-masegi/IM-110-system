uint8_t disp_MLSS_Meas_3(uint8_t flash_flg, uint8_t save, uint8_t batvol)
{
	...
	case MEAS_MODE_TR:
		...
		//200cm 超えは点滅停止、200.0 のまま表示 (Meas_2 と同じ仕様)。下限と FLT_MAX のみ点滅。
		...
		{
			//ローカル変数でクランプ (グローバル Transparency を破壊しない → check_stable がレンジ外を検出可能)
	...
	if (save == 0)
	{
		...
		LS027_disp_icon(5, 23, (uint8_t *)icon_save);
	}
	else if (save == 1)
	{
		LS027_disp_icon(0, 23, (uint8_t *)icon_save);
		LS027_disp_icon(10, 23, (uint8_t *)icon_complete);
	}
	else
	{
		LS027_disp_icon(0, 23, (uint8_t *)icon_release);
		LS027_disp_icon(10, 23, (uint8_t *)icon_complete);
	}
