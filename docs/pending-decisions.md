...
## 6. `STORE_VER` 1→2 の移行方式

### 現状

Page1/Page2 のフィールド配置が変わるため `STORE_VER` を 2 に上げる必要がある。
`store_valid()` は `ver != STORE_VER` を無効と判定するため、**Ver.1 のストアを載せた個体は新品既定へ落ちる**。

現在 Ver.1 で調整済みの実機は 1 台（LED duty 0.383 / ADC Span 4ch 調整済み / 清水ゼロ校正済み）。

### 裁定案

**マイグレーションを書かず、`RPF`（新品化）→ 再調整とする。**
移行コードは 1 回しか使われないうえ、旧レイアウトの解釈を誤ると調整値が壊れる。
実機 1 台なら再調整のほうが確実。

**影響**: 現在の調整値（LED duty・ADC Span・清水ゼロ・温度補正）は失われる。
書き込み前に `AMIR` と `P_RPG` でダンプを取っておけば手入力で戻せる。
