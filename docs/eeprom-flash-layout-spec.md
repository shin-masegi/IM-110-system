...
### 2-8. Page13 — 透視度 ゼロ・ベース・スパン校正

| off | B | フィールド | 内容 |
|---|---|---|---|
| 0 | 4 | `trzr` | 透視度 ゼロ 現場枠（現行 `store_tr_t` は 24 に `trzr_ship` 出荷時枠を追加済み、2026-08-03） |
| 4 | 8 | `pow_a, pow_b` | **旧** 出荷時ベース 累乗式。**2026-08-06 廃止 — 0 固定・不参照**（下記注） |
| 12 | 12 | `sp_a,sp_b,sp_c` | スパン校正 2 次 |
| 24 | 7 | `rsv[7]` | 予備（現行は 24 に `trzr_ship` 4B + rsv 3B） |
| 31 | 1 | `xor_cs` | ページ XOR |

温度補正は SS（Page10）を参照する。

> **2026-08-06 透視度 3点2次式化（累乗式廃止、`adboad.md` 22）**: TR の 1 次演算式は
> 保存しない。読出側が `trzr_ship` ＋ TR 出荷時3点調整 捕捉点（清水 100cm/60/30cm）から
> 毎回 2 次式を再フィットする（点が真実源）。`pow_a/pow_b` は書込 0 固定・読出不参照。
> 旧本体 FW は 0 を「未設定」として既定累乗にフォールバックするため後方安全。

### 2-9. Page14 — 透視度 校正後累乗（**2026-08-06 廃止 — 予約**）

| off | B | フィールド | 内容 |
|---|---|---|---|
| 0 | 8 | `pow_a_cal, pow_b_cal` | 旧 校正後 累乗式（廃止。0 固定・不参照） |
| 8 | 23 | `rsv[23]` | 予備 |
| 31 | 1 | `xor_cs` | ページ XOR |
...
| 13-15 | 共通設定 拡張用 予備 | 現行維持 |
| **16-18** | **後校正記録**（MLSS/SS/透視度 各1page 3件リング） | **2026-09-03 新設**（§3-3） |
...
### 3-3. 後校正記録（page16-18、2026-09-03 新設）

2点/3点校正の実測点選択を測定履歴（30件）から独立させた専用記録。
測定安定中画面の **MEM 長押し**で現在値を記録し、校正の「後校正値選択」画面（旧 記録選択
CAL_HSEL、タイトル `t_pcal_select`）はこの記録のみを参照する。

- page16 = MLSS（界面共用）/ page17 = SS / page18 = 透視度。各ページ独立の **3件リング**
  （書込は先頭シフト、最古を押し出し。`[0]` = 最新）
- ページレイアウト（32B）: offset 0/10/20 = 記録3件（各10B）、offset30 = マジック `0xC3`、
  offset31 = XOR CS（測定履歴と同式 `0xFF ^ bytes[0..30]`）
- 記録 1件（10B）: `year/month/day/hour/min`（各1B）＋ `value` int16
  （MLSS/SS = mg/L、透視度 = cm×10）＋ `raw_mv_x10` int16 ＋ 予備1B。
  水深は保存しない（読出し時 0 固定）。種別はページで分かれるため記録内に持たない
- 有効判定: マジック＋CS 一致、件数 = 先頭から連続する月 1..12（測定履歴と同じ）。
  不一致は「記録なし」扱い（新品・page0-125 再構築直後は 0 fill でここに落ちる）
- 実装: `Eeprom.c eep_read_postcal()/eep_write_postcal()`、RAM 像 = `pcal_his[3]`
