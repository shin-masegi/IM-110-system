...
> - 本書は改訂版レイアウト（`STORE_VER = 2`）を定義する。現行実装は `STORE_VER = 1`。
...
## 2. プローブ flash 統合ストア（`STORE_VER = 2`）
...
- Page1/Page2 のフィールド配置が変わるため、**`STORE_VER` を 2 に上げる**。
- `store_valid()` は `ver != STORE_VER` を無効と判定するため、**Ver.1 のストアを載せた個体は新品既定へ落ちる**。
  マイグレーションを書くか、実機を `RPF`（新品化）してから再調整するかを決めること。
  現時点で調整済みの実機は 1 台のみのため、**再調整で足りる想定**。
