...
| 16-18 | 共通設定 拡張用 予備 | 2026-09-03 に後校正記録へ割当したが **2026-09-07 撤去**（§3-3） |
...
### 3-3. 後校正記録（page16-18）— 撤去済み

2026-09-03（本体 Ver.0.43）に、2点/3点校正の実測点選択を測定履歴から独立させる目的で page16-18 を
種別ごとの 3件リング（マジック `0xC3`、offset31 XOR CS）に割り当てたが、**2026-09-07（本体 Ver.0.45）で撤去**し、
後校正の記録選択は従来どおり測定履歴（page81-125、30件）を参照する。page16-18 は共通設定 拡張用 予備へ戻す。
実機 EEPROM に残った記録データは FW から読まれない（マジック不一致として扱う経路も無く、単に未参照）。
旧レイアウトの詳細は git 履歴（system commit 28d57c4 時点の本節）を参照。
