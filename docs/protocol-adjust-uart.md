...
| `SADA,<N>` | **本体側 ADC 移動平均件数の変更**（2026-09-04 追加、本体 Ver.0.44。プローブ側 `P_SADA` の本体版）。N=1〜255、0 は 1 に矯正、範囲外・引数なしは `NG`。測定値・安定判断・校正捕捉が使う `ADC_mV_ave` の窓幅（MD 約500ms × N）が変わる。変更直後は次の MD 1 行でバッファを全件フィル。**RAM のみ・永続化しない。電源 OFF で既定 10 件へ戻る** | `SADA,<採用件数>\n` / `NG\n` | `Set_Main_AD_Ave` |
| `RAD` | 基板調整値取得 | — | 347 |
