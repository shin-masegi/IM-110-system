...
| **旧係数格納(probe_coef_t)** | `STORE_AD = 0x0801F800` | Page63 | 1408B(ver2) | 校正係数(相関式/モード別、真実源) | `coef_load()` | `coef_commit()` |
...
## 2. 旧係数格納(probe_coef_t) (Page63) — 校正係数 (protocol §3.2.6, ver2=1408B)

構造体 `probe_coef_t`(旧係数格納(probe_coef_t)) (IM_110.c:1088)。真実源=プローブ、本体=キャッシュ。
本体が `RPG` で読み出し、校正確定時に `WPG/WSC`(§3.2.6) 等で書き戻す。

- ヘッダ: `magic`(`COEF_MAGIC=0x494D3243 'IM2C'`), `ver`(`COEF_VER=2`), `checksum`(バイト総和)
...
初期値(未校正/新品): `coef_load()` の無効判定時 `memset(&g_coef,0,...)` + `g_coef_valid=0`。
...
coef_load();        // (6) STORE_AD → g_coef         ★flash読出
```

### 切り分け措置 (2026-07-19, 一時) — 実装済み【本体側のみ / プローブ無改変】

**方針**: 疑うのは **旧係数格納(probe_coef_t)(第2領域, 係数のプローブ格納, Ver.0.14〜)** と、それを触る
**本体側の通信**。**第1領域(PARAM)は Ver.0.12 から正常動作しているため対象外**。
プローブ側コード/フラッシュは **一切改変しない**。

**本体側** `IM-110/Core/Src/IM_110.c` PB_HS_STORE ハンドシェイク:
- 起動時の **`Probe_ReadCoef()`(RCF読出) と `Probe_MigrateToCoef()`(旧係数格納(probe_coef_t)書込) を両方コメントアウト**。
- 本体は **EEPROMキャッシュ校正を継続使用**(旧係数格納(probe_coef_t)導入前=Ver.0.12 相当の動作に近づく)。
- → 本体はプローブ 旧係数格納(probe_coef_t) に起動時アクセスしない。プローブは通常どおり起動(coef_load はプローブ内で
...
安定確認できたら復帰し、旧係数格納(probe_coef_t)アクセスと本体成否判定の恒久整合を設計する。

### 補足: 起動以外の本体側 旧係数格納(probe_coef_t)アクセス箇所 (今回は未変更)
起動時だけ切っても、運用中に触る経路が残っている。切り分けで残存するなら次はここも対象:
- `Normal.c:730` 相関式切替時に `Probe_ReadCoef()`(RCF読出→EEPROMキャッシュ同期)
- `Normal.c:1442 / 1678` 後校正確定時に 旧係数格納(probe_coef_t)へ書戻し(`WCF/WCS/WCFC`=プローブflash書込)
- `LinkSerial.c:265` 手動マイグレーション `Probe_MigrateToCoef()`

### 検証手順 (本体を書込後)
1. **本体のみ**書込(プローブは現行のまま)→ 電源再投入を**複数回**繰り返す。
2. 観察: 係数読み/計算値/起動タイトル表示が**毎回安定**するか(症状が変わらないか)。
3. 判定:
   - 安定した → **本体⇔プローブの 旧係数格納(probe_coef_t)通信が不安定の原因**。恒久対策(タイミング整合 or 構造見直し)へ。
   - 変わらず不安定 → 原因は起動時 旧係数格納(probe_coef_t)通信以外。運用中 旧係数格納(probe_coef_t)アクセス(上記補足)や UART 物理層へ切り分けを進める。

### 検証結果 (2026-07-19) — 不安定は消滅。原因=旧係数格納(probe_coef_t)通信で確定
...
本体⇔プローブ 旧係数格納(probe_coef_t)通信(プローブflashアクセス timing × 本体の成否判定 timing の噛み合わせ)が
不安定の元凶と確定。

### 恒久対策の選択肢 (次セッションで決定)
現状は「本体側 旧係数格納(probe_coef_t)アクセスを起動時だけ無効化+EEPROMキャッシュ運用」の暫定。恒久化は2択:
- **(1) 旧係数格納(probe_coef_t)通信を堅牢化して再有効化** — プローブ=真実源を維持。書込中flash-busy窓をまたぐ
  ACK/完了通知/リトライ整合、運用中アクセス(Normal.c:730/1442/1678)も同様に手当て。機能は残る。
- **(2) 係数の真実源を本体EEPROMに固定(旧係数格納(probe_coef_t)恒久オフ)** — 単純・確実。プローブが自前校正を運ぶ利点は失う。
  FUP透過ブリッジ等はそのまま。
