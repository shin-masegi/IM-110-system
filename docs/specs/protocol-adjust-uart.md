...
| `RAD` | なし | **全調整パラメータ読出**（2026-08-06 追加、ID-200T RAD 相当）。統合ストア25ページに記録される変数・設定値の RAM live/staging 一括ダンプ（読み取り専用・状態変更なし）。ヘッダ/SPAN/LED/水深/MLSS/SS/TR 係数/出荷時3点/現場校正点/**温度捕捉点 (`RAD,TC` 行 = %補正の6パラメータ、flash 確定前の値も見える)** | `RAD,ID:...` 〜 `RAD,END` 約30行（行別の意味は `docs/specs/RAD-AMIR-手順.md`） | `Probe_Store_DebugDumpAll` |
...
| `AMR` | なし | 捕捉バッファリセット（Mode_CF のみ。温度捕捉点は常設ストア管理となり対象外, 2026-08-02） | `OK\n` | 2081 |
| `AZC` | なし | **清水ゼロ校正**: 清水中の受光 Y0 を ゼロ基準 `*_ZR` に取得（暗時ADZR廃止後の測定ゼロ, §2改訂） | `OK\n`/`NG\n` | 2026-07-20 |
| `ACRI` | なし | **相関式初期化**: No.21-30 を出荷時基準(No.1=Mode_CF[0]) にコピー | `OK\n`/`NG\n` | 2026-07-20 |
| `AWD6` | なし | **水深スパン校正**: 6m 相当加圧時に Depth=6.0m になるよう傾き `k_depth` 調整（1点スロープ） | `OK\n`/`NG\n` | 2026-07-20 |
...
| `MD` | **表示中の測定値を返す**（2026-09-04 追加、本体 Ver.0.44）。読み取り専用・プローブへ送信しない。表示値は測定画面の規則どおり（MLSS=10mg/L 単位／SS=整数 1000/-99 クランプ／TR=小数1桁、100 超 `OVER`・0 以下 `-.-`）、無応答・未算出は `----`。表示値の後ろに移動平均後の受光 mV・Ref mV（`ADC_mV_ave`、MLSS=ch1/ch2・SS/TR=ch3/ch4、小数2桁）。水深（m、-0.99〜6.00）は MLSS モードのみ末尾に付加 | `MD,YYYY/MM/DD HH:MM:SS,<MLSS\|SS\|TR>,<表示値>,<受光mV>,<RefmV>[,<水深>]\n`（例 `MD,2026/09/04 10:30:15,MLSS,2340,1523.44,1361.02,0.12`） | `Send_Meas_Data` |
| `SADA,<N>` | **本体側 ADC 移動平均件数の変更**（2026-09-04 追加、本体 Ver.0.44。プローブ側 `P_SADA` の本体版）。N=1〜255、0 は 1 に矯正、範囲外・引数なしは `NG`。測定値・安定判断・校正捕捉が使う `ADC_mV_ave` の窓幅（MD 約500ms × N）が変わる。変更直後は次の MD 1 行でバッファを全件フィル。**RAM のみ・永続化しない。電源 OFF で既定へ戻る（既定 10 件。本体 Ver.0.45 で 1 件＝パススルーにしたが、Ver.0.46 でランプ方式をビルドスイッチ `PROBE_AD_AVE_RAMP`=0 で退避し 10 件へ戻した。プローブ側は `P_SADA,<N>`、protocol-rs232c.md §3.1／§8 0.6.9 補足）** | `SADA,<採用件数>\n` / `NG\n` | `Set_Main_AD_Ave` |
| `RAD` | 基板調整値取得 | — | 347 |
...
- 2026-09-04: `MD`（表示中測定値）/ `SADA,<N>`（本体側移動平均件数、非永続）を §4.2 に追加（本体 Ver.0.44）。
- 2026-09-07: `SADA,<N>` の既定を 10 → 1 件へ（本体 Ver.0.45。平均はプローブ側ランプ方式へ移行、protocol 0.6.7）。
- 2026-09-10: 本体 Ver.0.46 でランプ方式を退避し `SADA,<N>` の既定を 10 件へ戻した。`RAD`／`AMIR` の印刷用手順書 `docs/specs/RAD-AMIR-手順.md` を追加。
