...
| リポ | HEAD (push済) | 内容 |
|---|---|---|
| `IM-110`（本体, STM32L452） | `06a03fd` docs(eeprom): ミラー領域コメント訂正 | **T1: RPG/WPG/WSC＋§12↔計算グローバル写像＋EEPROMミラー(page61-76)＋Kiza削除**。ビルドOK・実機書込済 |
| `IM-110_Probe`（プローブ, STM32G070） | `9197660` docs: Page62失効コメント訂正 | **T1: 統合ストア一本化(Page63)＋RPG/WPG/WSC**、旧coef削除。ビルドOK・実機書込済(FUP) |
| `IM-110-system`（本リポ） | `68148c5` docs(spec): 仕様書 全55節 再生成 | 仕様・ログ・ハーネス。プログラム仕様書をコード裏取りで再生成 |

**2026-07-23 時点: 3リポとも clean・push済・実機書込済で疎通確認OK（大きな問題なし）**。**別マシンは3リポ同時 pull**（通信・計算は両側整合が前提）。旧HEAD: 本体`6ca22ee`/プローブ`2480d77`/system`5faa1dd`。

### 直近の作業と開いている項目（2026-07-23）
- **T1コア完了**（§4/§6-T1）: 係数を §12 統合ストア(512B/Page63)へ一本化、`RPG/WPG/WSC`(32Bページ転送)、本体EEPROMミラー、Kiza削除。**残**=起動3層ロード/OFF一括書戻し/last_update/起動時RPG自動発火（§6-T1 手順7-8, 未実装）。
