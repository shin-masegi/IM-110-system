...
> 最終更新: 2026-07-23 / このファイルは「今どうなっているか」を1枚で把握するための現状メモ。
...
| リポ | HEAD (push済) | 内容 |
|---|---|---|
| `IM-110`（本体, STM32L452） | `7bf6e72` fix(mirror): 起動時ミラーseed+共有既定ビルダー | **T1コア＋新品既定値の単一ソース化＋起動時ミラーseed**。ビルドOK・実機書込済(st-flash) |
| `IM-110_Probe`（プローブ, STM32G070） | `7fd46f5` fix(store): RPF既定に係数初期値(NaN根絶) | **T1コア＋RPFが係数ページに初期値を書込**、旧coef削除。ビルドOK・実機書込済(FUP) |
| `IM-110-system`（本リポ） | `c62b7b5` docs(spec): §12.1 新品既定値表 | 仕様・ログ・ハーネス。(本 handoff 更新でさらに進む) |

**2026-07-23 時点: 3リポとも clean・push済・実機書込済で疎通/計算確認OK**（本体計算値 MLSS=有限値 `V:1983.632`、NaN無し）。**別マシンは3リポ同時 pull**（通信・計算は両側整合＋共有 `probe_store.h` バイト一致が前提）。旧HEAD: 本体`06a03fd`/プローブ`9197660`/system`68148c5`。

### 直近の作業と開いている項目（2026-07-23）
...
- **⚠ 残（T1 手順7-8）**: 起動時3層ロードのうち **ミラー既定 seed は本日実装済**。**未実装**=ミラー→live 起動ロード・プローブID/最終更新日 調停・電源OFF一括書戻し・`last_update`(RTC日付)・起動時RPG自動発火(現状は相関式切替で発火/起動FSMは未接続)。→ この後は **T2(実機での一連調整・動作確認)**：`ALDA`(LED PWM 空中1700収束)から調整パイプラインを実機で通す。
