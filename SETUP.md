...
以下の項目は初回セットアップ時に検証・記入してください。以降、基板リビジョンや仕様が変わったとき
（新しい `.NET` / 回路図 PDF が `Materials/` 直下に来たとき）も同じ手順で再検証する。

- §6 ビルド・テストコマンド: 各サブリポの Makefile から導出
- §4 ハードウェア要点: **§3.5「ハードウェア仕様の真実源」ルールに従い、必ず各サブリポ `Materials/` 配下の
  回路図 PDF + ネットリスト (`.NET`) を根拠とする。FW (`.ioc` / `main.c`) を真実源にしてはならない。**
  更新時は根拠ファイル名（と可能ならネット名 / コンポーネント ID）を §4 本文に併記する。
- (できれば) 各サブリポにも `CLAUDE.md` を設置して、そのリポ固有のルールを書く

> 注: §4 の内容は実装（FW コード）ではなく回路実態（PDF + NET）から導出される。FW 側と食い違いが
> 見つかった場合は §3.5 に従って回路図側を真実とし、FW の CubeMX 設定ミスを疑って本体/プローブ側の
> `CLAUDE.md` / コードを修正する順序で処理する。
